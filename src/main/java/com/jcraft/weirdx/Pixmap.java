...
class Pixmap extends Drawable {
    ...
    Pixmap(int id, Drawable d,
        ...
        if (img == null) System.err.println("createImage() returned null");
        img.flush();
    ...
    static Pixmap createPixmap(int id, Drawable d,
        ...
        Pixmap p = null;
        if (depth == 1) {
            p = new Pixmap1(id, d, width, height);
        } else if (depth == 16) {
            p = new Pixmap16(id, d, width, height);
        } else {
            p = new Pixmap(id, d, width, height, depth);
        }
        Resource.add(p);
        return p;
    }
...
class Pixmap16 extends Pixmap {
    ...
    throws IOException {
        ...
        if (depth == 1) {
            ...
            int resth = dheight;
            if (height < dsty + dheight) {
                resth = (short)(height - dsty);
            }
            resth += dsty;
            int restw = dwidth;
            if (width < dstx + dwidth) {
                restw = (short)(width - dstx);
            }
            restw += dstx;
...
class ResizablePixmap extends Pixmap implements Resizable {
    ...
    static Pixmap createPixmap(int id, Drawable d,
        ...
        Pixmap p = null;
        if (depth == 1) {
            p = new ResizablePixmap1(id, d, width, height);
        } else if (depth == 16) {
            p = new ResizablePixmap16(id, d, width, height);
        } else {
            p = new ResizablePixmap(id, d, width, height, depth);
        }
        Resource.add(p);
        return p;
    }
