...
class Window extends Drawable {
    ...
    static void installDDXWindow(String name) {
        Class c = null;
        try {
            if (name.startsWith("com.jcraft.weirdx.")) c = Class.forName(name);
        } catch (Exception e) {
            System.err.println(e);
        }
    ...
    throws IOException {
        ...
        try {
            if (screen.windowmode != WeirdX.InBrowser && prnt == screen.root) {
                final xyz.pugduddly.enderX.ui.Window frame = getFrame();
                /*
	  ...
	  ((JFrame)frame).getContentPane().setLayout(null);        
  	  ((JFrame)frame).setResizable(false);                                 
                ...
                if (frame instanceof xyz.pugduddly.enderX.ui.Window) {
                    //((xyz.pugduddly.enderX.ui.Window)frame).setMenuBar(null);                                  
                    ((xyz.pugduddly.enderX.ui.Window) frame).setResizable(true);
                }
    ...
    static final void reqCreateWindow(Client c) throws IOException {
        ...
        //System.out.println(foo + ", " + prnt);

        x = io.readShort(); // Tbh this isn't useful, as EnderX manages the windows
        y = io.readShort(); // ^
        width = io.readShort();
        height = io.readShort();
        bwidth = io.readShort();
    ...
    int sendEvent(Event event, int count, int filter,
        ...
        if ((filter == Event.CantBeFiltered) ||
            ...
            attempt = 1;
            try {
                attempt = client.sendEvent(event, count, eventMask, filter, grab);
            } catch (Exception e) {}
        ...
        if (filter != Event.CantBeFiltered) {
            if ((type & Event.EXTENSION_EVENT_BASE) != 0) {
                ...
                other = (Clients)(optional == null ? null : optional.otherClients);
            }
    ...
    static xyz.pugduddly.enderX.ui.Window owner = new xyz.pugduddly.enderX.ui.Window("WeirdX");
    xyz.pugduddly.enderX.ui.Window frame = null;

    static private final int poolsize = 5;
    static xyz.pugduddly.enderX.ui.Window[] frames = new xyz.pugduddly.enderX.ui.Window[poolsize];

    xyz.pugduddly.enderX.ui.Window getFrame() {
        if (hasFrame()) return optional.frame;
        
        if (this.parent != screen.getRoot()) {
            return null;
        }
        ...
        if (screen.windowmode == WeirdX.RootlessWM &&
            ...
            (attr & (1 << 16)) == 0) {
            optional.frame = new xyz.pugduddly.enderX.ui.Window("WeirdX");
            optional.frame.addMouseListener((java.awt.event.MouseListener) ddxwindow);
            optional.frame.addKeyListener((java.awt.event.KeyListener) ddxwindow);
            //optional.frame.setVisible(true);
            //System.out.println("Create window");
            return optional.frame;
        }

        synchronized(Window.class) {
            xyz.pugduddly.enderX.ui.Window foo = null;
    ...
    void delFrame() {
        ...
        if (screen.windowmode == WeirdX.RootlessWM &&
            optional.frame instanceof xyz.pugduddly.enderX.ui.Window) {
            optional.frame.removeMouseListener((java.awt.event.MouseListener) ddxwindow);
            optional.frame.removeKeyListener((java.awt.event.KeyListener) ddxwindow);
                ...
                if (((mask & CWX) != 0) || ((mask & CWY) != 0) ||
                    ...
                    if (((mask & CWWidth) != 0) || ((mask & CWHeight) != 0) ||
                        ...
                        if (win.screen.windowmode != WeirdX.InBrowser && win.hasFrame()) {
                            xyz.pugduddly.enderX.ui.Window frame = win.getFrame();
                            //frame.validate();
                            /*Insets insets=frame.getInsets();                         
    ...
    throws IOException {
        ...
        synchronized(LOCK) {
            ...
            if (screen.windowmode != WeirdX.InBrowser && hasFrame()) {
                xyz.pugduddly.enderX.ui.Window frame = getFrame();
    ...
    throws IOException {
        ...
        out = (fromWin == 0) ? NotifyDetailNone : NotifyPointerRoot; in = (toWin == 0) ? NotifyDetailNone: NotifyPointerRoot;
    ...
    private void addComponentListener(xyz.pugduddly.enderX.ui.Window foo) {
        final xyz.pugduddly.enderX.ui.Window frame = foo;
                    ...
                    if (frame_width != rectangle.width ||
                        ...
                        synchronized(Window.LOCK) {
                            try {
                                ...
                                int ww = rectangle.width - borderWidth * 2;
                                int hh = rectangle.height - borderWidth * 2;
                                if (ww > 0 && hh > 0) {
...
class WindowOpt {
    ...
    xyz.pugduddly.enderX.ui.Window frame;
}
