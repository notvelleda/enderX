 ...
 * Modifications (C) 2018 Pugduddly (pg)
 *
 * 02-Aug-2000: ms modified key events to handle the ALT_GRAPH key
 * 29-Aug-2000: ms modified this class to be free from Swing APIs.
 * 
 * 6/23/2018: pg did a lot of fucking stuff to this class lmao
 *
...
class DDXWindowImp extends Component
implements DDXWindow, MouseListener, MouseMotionListener, KeyListener {
    ...
    private Rectangle exposed = new Rectangle();

    DDXWindowImp() {}

    public Image createImage(int w, int h) {
        Image img = (Image) new BufferedImage(w, h, BufferedImage.TYPE_INT_ARGB);
        this.prepareImage(img, w, h, null);
        return img;
    }
            ...
            if (window.screen.windowmode != WeirdX.InBrowser && window.hasFrame()) {
                synchronized(Window.LOCK) {
                    ...
                    if (p != null && p.type == 31) {
                        ...
                        ((xyz.pugduddly.enderX.ui.Window) frame).setTitle(title);
                    }
                }
                frame.add(this);
		      ...
		      window.height+window.borderWidth*2+
                ...
                frame.setSize(window.width + window.borderWidth * 2, window.height + window.borderWidth * 2);
    ...
    public void setSize(int w, int h) {
        ...
        super.setPreferredSize(new Dimension(w + 2 * bw, h + 2 * bw));
    ...
    public void paintComponent(Graphics g) {
        ...
        //System.out.println(this + ".paint(): " + this.components.size() + " child components");
    ...
    public void mousePressed(MouseEvent e) {
        if (threeButton) {
            ...
            if (threeBstate == sp/* || threeBstate==sppr*/) {
                threeBPressed = null;
                threeBstate = spp;
    ...
    public void mouseDragged(MouseEvent e) {
        ...
        event.mkMotionNotify(1, window.screen.rootId,
    ...
    public void keyPressed(KeyEvent e) {
        ...
        // Easter Egg... (won't work tho bc screen root is never shown under normal operation, I left it in just because)
        if (window == window.screen.root && px > 0 && px < 3 && py > 0 && py < 3) {
            if (((e.getModifiers() & InputEvent.CTRL_MASK) != 0) &&
                ...
                System.out.println("EnderX " + xyz.pugduddly.enderX.EnderX.VERSION + ", WeirdX " + WeirdX.releaseNumber);
            }
        }
    ...
    public Graphics getGraphics() {
        if (window.clss == InputOnly) return super.getGraphics();
    ...
    public void copyArea(int sx, int sy, int w, int h, int dx, int dy) {
        ...
        if ((dx <= 0 || w <= dx) && (dy <= 0 || h <= dy)) {
            ...
            if (0 < dy && dy < h) {
                ...
                ssy += dy;
                if (sy < ssy) {
                    offg.copyArea(sx, sy, w, ssy - sy, dx, dy);
                }
            } else if (0 < dx && dx < w) {
                int ssx = sx + w - dx;
                while (true) {
                ...
                ssx += dx;
                if (sx < ssx) {
                    offg.copyArea(sx, sy, ssx - sx, h, dx, dy);
                }
            }
        }
    }
